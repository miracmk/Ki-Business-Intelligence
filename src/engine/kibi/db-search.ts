...
export async function searchEntityData(params: { entityId: string; question: string; connectionId?: string }): Promise<string> {
  ...
  if (result.error) return `Sorgu sırasında hata: ${result.error}`
  if (!result.rows.length) return 'Bu entity için CRM verisi bulunamadı.'
...
export async function getPlatformStats(question: string): Promise<string> {
  ...
  const topEntities = tokenRows.rows.map((row: any) => `${row.entity_id} → ${row.tokens} token`) .join('\n')
