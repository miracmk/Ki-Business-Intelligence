...
import { kibiSupportTickets, kibiSupportMessages, } from '../../../db/schema.js';
// ── Helpers ───────────────────────────────────────────────────────────────────
async function getEntityChannelConfig(tenantId) {
    const tenant = await db.query.tenants.findFirst({
        where: (t, { eq }) => eq(t.id, tenantId),
        columns: { settings: true },
    });
    return (tenant?.settings?.channels ?? {});
}
async function getEntityByTenantId(tenantId) {
    return db.query.kibiEntities.findFirst({
        where: (t, { eq }) => eq(t.entityId, tenantId),
    });
}
// ── Send helpers (entity-scoped) ──────────────────────────────────────────────
async function sendViaWhatsApp(to, text, creds) {
...
async function sendViaTelegram(chatId, text, botToken) {
...
async function sendViaInstagram(recipientId, text, accessToken) {
...
async function sendViaEmail(to, subject, body, smtpCfg) {
...
export async function routeReplyToExternal(ticketId, replyText) {
    const ticket = await db.query.kibiSupportTickets.findFirst({
        where: (t, { eq }) => eq(t.id, ticketId),
    });
    if (!ticket?.externalContactId || !ticket?.contactChannel)
        return;
...
async function assignRoundRobin(entityId) {
...
async function notifyAgent(agentUserId, entityId, ticket, firstMessage, entityTenantId) {
    ...
    if (agent.channelPreference === 'telegram' && agent.telegramChatId && ch.telegram?.bot_token) {
        ...
        await sendViaWhatsApp(agent.waPhone, notifText, ch.whatsapp);
    }
...
function buildAutoReply(entityName, channel) {
...
// ── Main pipeline ─────────────────────────────────────────────────────────────
export async function processExternalMessage(entityTenantId, channel, externalContactId, // WA phone / TG chat_id / IG user_id / email
contactName, messageText) {
    ...
    let ticket = await db.query.kibiSupportTickets.findFirst({
        where: (t, { and, eq }) => and(eq(t.entityId, entity.id), eq(t.externalContactId, externalContactId), eq(t.contactChannel, channel), eq(t.status, 'open')),
    });
    ...
    if (isNewTicket) {
        ...
        WHERE id = ${ticket.id}
      `);
            await notifyAgent(agentUserId, entity.id, ticket, messageText, entityTenantId).catch(console.error);
        }
        else {
...
// ── Auto-reply helper ─────────────────────────────────────────────────────────
async function sendAutoReply(channel, contactId, replyText, ch, _entityName) {
    if (channel === 'whatsapp' && ch.whatsapp?.phone_number_id && ch.whatsapp?.access_token) {
        await sendViaWhatsApp(contactId, replyText, ch.whatsapp);
    }
    else if (channel === 'telegram' && ch.telegram?.bot_token) {
