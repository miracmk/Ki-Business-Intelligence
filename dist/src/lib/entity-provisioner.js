 ...
 */
export function deriveSchemaName(slug) {
 ...
 */
export async function provisionEntity(kibiEntityId, slug) {
    ...
    try {
        ...
        const exists = await client.query(`SELECT EXISTS(SELECT 1 FROM information_schema.schemata WHERE schema_name = $1)`, [schemaName]);
 ...
 */
export async function queryEntitySchema(schemaName, sql, params = []) {
    const pool = new Pool({ connectionString: env.DATABASE_URL, max: 1 });
    const client = await pool.connect();
    try {
 ...
 */
export async function getEntitySchema(kibiEntityId) {
    const rows = await db
 ...
 */
export async function getEntityDataSummary(schemaName) {
    const pool = new Pool({ connectionString: env.DATABASE_URL, max: 1 });
    const client = await pool.connect();
    try {
        await client.query(`SET search_path TO "${schemaName}", public`);
        const [contacts, companies, deals, products, orders, staff, invoices, expenses] = await Promise.all([
            client.query(`SELECT COUNT(*) FROM crm_contacts  WHERE deleted_at IS NULL`),
            client.query(`SELECT COUNT(*) FROM crm_companies WHERE deleted_at IS NULL`),
            client.query(`SELECT COUNT(*), COALESCE(SUM(deal_value),0) AS total_value FROM crm_deals WHERE deleted_at IS NULL AND stage NOT IN ('won','lost')`),
            client.query(`SELECT COUNT(*), SUM(CASE WHEN reorder_point IS NOT NULL AND available_quantity <= reorder_point THEN 1 ELSE 0 END) AS low_stock FROM erp_products WHERE deleted_at IS NULL AND is_active = TRUE`),
            client.query(`SELECT COUNT(*), COALESCE(SUM(total),0) AS total FROM erp_orders WHERE order_date >= CURRENT_DATE - INTERVAL '30 days'`),
            client.query(`SELECT COUNT(*), SUM(CASE WHEN status = 'on_leave' THEN 1 ELSE 0 END) AS on_leave FROM erp_staff WHERE status != 'terminated'`),
            client.query(`SELECT COALESCE(SUM(remaining_amount),0) AS receivable, COALESCE(SUM(CASE WHEN due_date < CURRENT_DATE AND status NOT IN ('paid','cancelled') THEN remaining_amount ELSE 0 END),0) AS overdue FROM acc_invoices WHERE invoice_type = 'sale'`),
            client.query(`SELECT COALESCE(SUM(total_amount),0) AS total FROM acc_expenses WHERE expense_date >= DATE_TRUNC('month', CURRENT_DATE)`),
        ]);
        return {
            crm: {
...
//# sourceMappingURL=entity-provisioner.js.map