...
export class PostgreSqlAdapter extends CrmAdapter {
    ...
    async getModuleFields(module) {
        ...
        if (c.fieldsTable) {
            ...
            try {
                ...
                const { rows } = await client.query(`SELECT * FROM "${c.fieldsTable}" WHERE "${moduleCol}" = $1`, [module]);
                return rows.map(r => ({
                    apiName: String(r[apiNameCol] ?? ''),
                    label: String(r[labelCol] ?? r[apiNameCol] ?? ''),
    ...
    async search(params) {
        ...
        if (c.dataTable) {
            ...
            try {
                ...
                const { rows } = await client.query(`SELECT * FROM "${c.dataTable}" WHERE "${moduleCol}" = $1 LIMIT $2 OFFSET $3`, [params.module, perPage, offset]);
                return rows.map(row => {
                    let recordData = {};
                    if (dataCol && typeof row[dataCol] === 'object') {
                        recordData = row[dataCol];
                    }
                    else {
                        recordData = row;
                    }
        ...
        try {
            ...
            const { rows } = await client.query(`SELECT ${cols} FROM "${params.module}" LIMIT $1 OFFSET $2`, [perPage, offset]);
    ...
    async getRecord(module, id) {
        const client = await this.pool.connect();
        try {
            const { rows } = await client.query(`SELECT * FROM "${module}" WHERE id = $1 LIMIT 1`, [id]);
    ...
    async createRecord(module, data) {
        ...
        try {
            ...
            const { rows } = await client.query(`INSERT INTO "${module}" (${cols.map(c => `"${c}"`).join(', ')}) VALUES (${places}) RETURNING *`, vals);
    ...
    async updateRecord(module, id, data) {
        ...
        try {
            ...
            const { rows } = await client.query(`UPDATE "${module}" SET ${sets} WHERE id = $${vals.length + 1} RETURNING *`, [...vals, id]);
    ...
    async *streamTable(tableName, batchSize = 500) {
        const client = await this.pool.connect();
        try {
            let offset = 0;
            while (true) {
                const { rows } = await client.query(`SELECT * FROM "${tableName}" LIMIT $1 OFFSET $2`, [batchSize, offset]);
                if (rows.length === 0)
                    break;
    ...
    async getTableCount(tableName) {
        const client = await this.pool.connect();
        try {
            const { rows } = await client.query(`SELECT COUNT(*) AS n FROM "${tableName}"`);
