...
// ── Main entry point ──────────────────────────────────────────────────────────
export async function runEntityEtl(connectionId) {
...
async function runConnectorEtl(pool, connectionId, schemaName, crmType, config) {
    const result = { ok: true, rows: 0, mirrored: 0, tables: [] };
    const client = await pool.connect();
    let modules = [];
    try {
        const { rows } = await client.query(`SELECT DISTINCT module_api_name FROM public.crm_records WHERE connection_id = $1`, [connectionId]);
    ...
    for (const module of modules) {
        ...
        if (!mapping || !mapping.targetTable)
            continue;
        ...
        while (true) {
            ...
            let records = [];
            try {
                const { rows } = await batchClient.query(`SELECT data, crm_id FROM public.crm_records WHERE connection_id = $1 AND module_api_name = $2 LIMIT $3 OFFSET $4`, [connectionId, module, MIRROR_BATCH, offset]);
                records = rows;
            }
            finally {
                batchClient.release();
            ...
            if (!result.tables.includes(mapping.targetTable))
                result.tables.push(mapping.targetTable);
            if (records.length < MIRROR_BATCH)
                break;
            offset += MIRROR_BATCH;
        }
    }
...
function applyConnectorMapping(data, mapping, connectionId, sourceId, sourceType) {
    const row = {
    ...
    for (const fieldMap of mapping.fields) {
        ...
        switch (fieldMap.transform) {
            ...
            case 'phone_e164':
                ...
                if (!transformed.startsWith('+') && String(transformed).length === 10)
                    transformed = '+90' + transformed;
                break;
            case 'country_iso':
        ...
        if (fieldMap.targetField === 'custom_fields') {
            ...
            if (isDate) {
                try {
                    row[fieldMap.targetField] = new Date(str);
                }
                catch { /* skip */ }
            }
            else {
    ...
    if (Object.keys(custom).length > 0)
        row['custom_fields'] = JSON.stringify(custom);
    if (mapping.targetTable === 'crm_contacts' && !row['full_name']) {
        const fn = (row['first_name'] ?? '');
        const ln = (row['last_name'] ?? '');
        row['full_name'] = `${fn} ${ln}`.trim() || row['email'] || sourceId;
    }
    if (mapping.targetTable === 'crm_companies' && !row['name'])
        row['name'] = sourceId;
    if (mapping.targetTable === 'crm_deals' && !row['title'])
        row['title'] = sourceId;
    return row;
}
// ── Ensure a specific entity table exists (create from template if missing) ───
const ENTITY_TABLE_DDL = {
    crm_contacts: (s) => `
...
async function ensureEntityTable(pool, schemaName, tableName) {
...
async function ensureEtlTables(pool, schemaName) {
    const client = await pool.connect();
    try {
...
async function runCrmRecordsEtl(pool, connectionId, schemaName, crmType, tenantId) {
    const result = { ok: true, rows: 0, mirrored: 0, tables: [] };
    ...
    for (const module of modules) {
        ...
        while (true) {
            ...
            if (targetTable) {
                ...
                if (!result.tables.includes(targetTable))
                    result.tables.push(targetTable);
            }
            if (records.length < MIRROR_BATCH)
                break;
            offset += MIRROR_BATCH;
...
async function runPostgresEtl(pool, connectionId, schemaName, creds, _tenantId) {
        ...
        for (const mod of externalModules) {
            ...
            if (targetTable) {
                ...
                while (true) {
                    ...
                    let mirrorRows = [];
                    try {
                        const { rows } = await batchClient.query(`SELECT raw_data, source_id FROM "${schemaName}".crm_raw_mirror
               WHERE connection_id = $1 AND source_table = $2 AND normalized_at IS NULL
               LIMIT $3 OFFSET $4`, [connectionId, mod.apiName, MIRROR_BATCH, offset]);
                        mirrorRows = rows;
                    }
                    finally {
                        batchClient.release();
                    ...
                    for (let i = 0; i < mirrorRows.length; i += AI_BATCH_SIZE) {
                        ...
                        for (let j = 0; j < batch.length; j++) {
                            ...
                            if (row) {
                                ...
                                // Mark as normalized
                                await pool.query(`UPDATE "${schemaName}".crm_raw_mirror SET normalized_at = NOW(), target_table = $1, target_id = $2
                   WHERE connection_id = $3 AND source_table = $4 AND source_id = $5`, [targetTable, targetId, connectionId, mod.apiName, batch[j].source_id]);
                                result.rows++;
                            }
                        }
                    }
                    if (mirrorRows.length < MIRROR_BATCH)
                        break;
                    offset += MIRROR_BATCH;
                }
                if (!result.tables.includes(targetTable))
...
async function mirrorCrmMetadata(pool, schemaName, connectionId, crmType) {
    ...
    const client = await pool.connect();
    try {
        await client.query(`INSERT INTO "${schemaName}".entity_settings (key, value, updated_at)
       VALUES ('crm_source_metadata', $1::jsonb, NOW())
       ON CONFLICT (key) DO UPDATE SET value = $1::jsonb, updated_at = NOW()`, [JSON.stringify(meta)]);
    }
    finally {
        client.release();
    }
}
async function mirrorPgMetadata(pool, schemaName, connectionId, adapter, modules) {
    const fieldsPerModule = {};
    ...
    const meta = {
        ...
        fields: Object.entries(fieldsPerModule).flatMap(([mod, fields]) => fields.map(f => ({ module: mod, apiName: f.apiName, label: f.label, dataType: f.dataType }))),
    };
    const client = await pool.connect();
    try {
        await client.query(`INSERT INTO "${schemaName}".entity_settings (key, value, updated_at)
...
async function bulkMirrorRecords(pool, schemaName, connectionId, sourceTable, records) {
    if (records.length === 0)
        return;
    const client = await pool.connect();
    try {
        for (const rec of records) {
            await client.query(`INSERT INTO "${schemaName}".crm_raw_mirror (connection_id, source_table, source_id, raw_data)
         VALUES ($1, $2, $3, $4::jsonb)
         ON CONFLICT (connection_id, source_table, source_id)
         WHERE source_id IS NOT NULL
         DO UPDATE SET raw_data = $4::jsonb, synced_at = NOW()`, [connectionId, sourceTable, rec.sourceId || null, JSON.stringify(rec.data)]).catch(() => {
                // If unique constraint fails (e.g. source_id is null), do plain insert
                return client.query(`INSERT INTO "${schemaName}".crm_raw_mirror (connection_id, source_table, source_id, raw_data)
           VALUES ($1, $2, $3, $4::jsonb) ON CONFLICT DO NOTHING`, [connectionId, sourceTable, rec.sourceId || null, JSON.stringify(rec.data)]);
            });
        }
    }
    finally {
...
async function aiNormalizeBatch(gateway, records, sourceType, moduleName) {
    if (records.length === 0)
        return [];
...
Çıktı: Sadece düzeltilmiş JSON dizisini döndür (markdown/açıklama OLMADAN), aynı sırada, aynı uzunlukta.`;
    try {
        ...
        if (jsonStart === -1 || jsonEnd === 0)
            return records; // fallback to original
...
function crmTypeToEntityTable(crmType, module) {
    return CRM_TYPE_MODULE_TABLE[crmType]?.[module] ?? null;
}
function buildEntityRow(data, crmType, module, connectionId, crmId) {
    ...
    if (table === 'crm_contacts') {
        if (!row['full_name']) {
            const fn = (row['first_name'] ?? '');
            const ln = (row['last_name'] ?? '');
            row['full_name'] = `${fn} ${ln}`.trim() || row['email'] || `Contact-${crmId.slice(0, 8)}`;
        }
        if (module === 'Leads')
            row['contact_type'] = 'lead';
    }
...
function detectPgTargetTable(tableName, columns) {
    ...
    if (colSet.has('email') || colSet.has('eposta'))
        return 'crm_contacts';
    if (colSet.has('invoice_number') || colSet.has('fatura_no'))
        return null; // skip invoice tables
    return null;
}
function buildPgEntityRow(normalized, original, columns, connectionId, sourceId, targetTable) {
    ...
    for (const col of columns) {
        ...
        for (const [regex, dest] of patterns) {
            if (regex.test(col)) {
                if (dest === 'created_at' || dest === 'updated_at' || dest === 'expected_close_date') {
                    try {
                        mapped[dest] = new Date(String(val));
                    }
                    catch { /* skip */ }
                }
                else {
    ...
    if (Object.keys(unmapped).length > 0)
        mapped['custom_fields'] = JSON.stringify(unmapped);
    // Required field fallbacks
    if (targetTable === 'crm_contacts' && !mapped['full_name']) {
        const fn = (mapped['first_name'] ?? '');
        const ln = (mapped['last_name'] ?? '');
        mapped['full_name'] = `${fn} ${ln}`.trim() || mapped['email'] || sourceId;
        if (!mapped['full_name'])
            return null;
    }
...
// ── Upsert helper ─────────────────────────────────────────────────────────────
async function upsertEntityRow(pool, schemaName, table, row, conflictCol) {
...
//# sourceMappingURL=entity-etl.js.map