...
import { kibiSupportTickets, kibiSupportMessages, kibiSupportKnowledge } from '../../../db/schema.js';
...
export async function processNewTicket(params) {
...
async function refineTicket(ticketId, message) {
    const ticket = await db.query.kibiSupportTickets.findFirst({ where: (t, { eq }) => eq(t.id, ticketId) });
    if (!ticket)
        return;
...
async function refineAnswer(rawAnswer, ticket) {
...
export async function sendTicketAnswer(ticketId, answer, channel) {
    const ticket = await db.query.kibiSupportTickets.findFirst({ where: (t, { eq }) => eq(t.id, ticketId) });
    if (!ticket)
        return;
    if (channel === 'email') {
...
//# sourceMappingURL=support-pipeline.js.map