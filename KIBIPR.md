...
### YFZ 34.5 — Customer Service Management Add-on (Faz 5a) ✅ (2026-06-25)
- [x] **Schema:** `support_sla_policies` / `support_tickets` / `support_ticket_messages` Base entity DDL'ine eklendi (`db/entity-schema-template.sql`, gelecek tenant'lar için) + `db/migrations/0018_support_tables.sql` (mevcut provision edilmiş tüm entity şemalarına `DO $$ ... FOR r IN SELECT entity_db_schema FROM kibi_entities WHERE is_provisioned ... EXECUTE format(...)` ile geriye dönük uygulandı, `psql` ile çalıştırıldı).
  - **Bulunan ön-var olan hata (düzeltilmedi, kapsam dışı):** `entity-schema-template.sql`'deki orijinal index isimlendirme deseni (`":schema"_tablename_idx`) **geçersiz SQL** — quoted identifier'a bitişik karakterler eklemek syntax error verir (doğrudan test edildi). Canlı `entity_ki_business` şemasındaki mevcut index'ler farklı (muhtemelen daha eski/elle düzeltilmiş) bir mekanizmayla oluşmuş. Yeni eklenen tablolarım için index isimlerini şema-önekisiz yazdım (`support_tickets_status_idx` — Postgres index'i otomatik doğru şemaya yerleştirir, önek gereksiz). Dosyadaki diğer (crm/erp/acc) index satırları aynı kusuru taşıyor ama bu fazın kapsamı dışında, dokunulmadı.
  - **Mimari karar:** Add-on tabloları Base DDL'de yaşar (ek provisioning maliyeti yok); native CRUD/UI erişimi `entity_module_entitlements` ile API katmanında kapılanır, şema varlığı ile değil — Personnel Management'taki (`erp_staff`) ile aynı desen.
- [x] **Backend:** `src/lib/entity-provisioner.ts`'e `applySchemaIncrement()` eklendi (gelecekte programatik kullanım için altyapı — bu fazda DB'ye uygulama migration dosyası+psql ile yapıldı, kullanıcı kararıyla).
  - `src/api/routes/customer-service.ts` (yeni dosya): `app.addHook('preHandler', ...)` ile **dosya genelinde** `addon_customer_service` entitlement kontrolü (admin/supervisor bypass) — tek tek her route'a tekrar etmek yerine merkezi gate. Entitlement yoksa 402.
  - Endpoint'ler: SLA politikaları (GET/POST), biletler (GET/POST/PUT/DELETE→soft-close), bilet mesaj akışı (GET/POST) — SLA politikası seçilince `first_response_due_at`/`resolution_due_at` otomatik hesaplanır; agent mesajı gönderilince `first_responded_at` ilk kez set edilir.
  - `server.ts`'e `/api/v1/customer-service` prefix'iyle kayıtlı.
- [x] **Frontend:** `frontend/src/pages/CustomerService.tsx` (yeni sayfa) — entitlement durumuna göre iki mod: **kilitli** (kilit ikonu + "Modülü Etkinleştir" CTA, `POST /entitlements/addon_customer_service/activate` çağırır, varsayılan ücretsiz/$0 — gerçek fiyatlandırma/ödeme akışı kapsam dışı, gelecek bir faz) veya **aktif** (bilet tablosu + yeni bilet modalı + mesaj thread modalı).
...
## FAZ 5 — Event-Driven Hooks + Declarative Rule Engine ✅ TAMAMLANDI (2026-06-26)

### FAZ 5.1 — BullMQ
- `bullmq` eklendi (`package.json`). `src/lib/queue/index.ts`: **kasıtlı olarak ayrı** bir
  ioredis bağlantısı (`maxRetriesPerRequest: null`) — paylaşılan `lib/redis.ts` instance'ı
  (`maxRetriesPerRequest: 3`) BullMQ Worker'a verilirse startup'ta atıyor, bu yüzden ayrı
  connection kuruldu. Queue adı `kibi-actions` (BullMQ queue isimlerinde `:` YASAK — ilk
  denemede `kibi:actions` ile "Queue name cannot contain :" hatası alındı, düzeltildi).
- `src/workers/index.ts`: ayrı worker process'i (`npm run worker`), 5 action tipini
  (`update_field`,`webhook`,`email`,`run_function`,`require_approval`) job name'e göre dispatch
  eder. `require_approval`/`run_function` şu an stub (Faz 6/7'ye köprü, log + no-op).
- `docker-compose.yml`: yeni `ki_worker` servisi — ki_api ile aynı image/volume düzeni, ama
  `ki_bi_network`'e (npm_default/external_network YOK — inbound trafiğe ihtiyacı yok) bağlı.
  **Bulunan tuzak:** `config/env.ts` zod şemasını import anında TÜMÜYLE doğruluyor (APP_URL,
  JWT_SECRET, WA_* dahil — worker bunların hiçbirini kullanmasa da). İlk denemede worker'a
  yalnızca DB/Redis/SMTP env'leri verilince crash oldu; ki_api'nin TÜM environment bloğu
  kopyalanınca düzeldi.

### FAZ 5.2 — Lifecycle Hooks
- `src/lib/hooks/lifecycle.ts`: `runHooks('afterSave', ctx)` + `registerAfterSaveHook(fn)`.
  `beforeSave` şimdilik no-op (Faz 6 blueprint gating için ayrılmış). Her hook best-effort —
  hata `console.error`'a düşer, save'i bloklamaz/rollback yapmaz.
  **FAZ 4'ten taşınan ders uygulandı:** hook'lar `buildInsert`/`buildUpdate` çıktısını
  SARMALAR, içine girip değiştirmez — `ctx.record`'u sadece response enrichment için mutate
  eder (örn. AI alanı sonucu), gerçek DB yazımı ayrı, kendi merge UPDATE'iyle olur.
- `src/lib/hooks/handlers/ai-field-hook.ts`: FAZ 4.5'in `runAiFields`'ını sarmalar —
  **doğrudan çağrı YERİNE** hook zincirine taşındı (crm-native.ts'teki 6 doğrudan
  `runAiFields(...)` çağrısı silinip `runHooks('afterSave', ...)` ile değiştirildi — ikisi
  birden kalsaydı AI alanı iki kez hesaplanırdı).
- `src/lib/hooks/handlers/rule-engine-hook.ts`: FAZ 5.4 evaluator'ı sarmalar.
- `src/lib/hooks/register.ts`: side-effect import (server.ts'te), iki hook'u da kaydeder.
- `crm-native.ts`'in 4 modülünün (contacts/companies/deals/activities) create+update'i
  (8 call site) artık `runHooks('afterSave', { ..., trigger:'on_create'|'on_update', record })`
  çağırıyor. **Re-entrancy:** `update_field` worker'ı `queryEntitySchema`'yı doğrudan çağırır,
  native route'a hiç girmez → afterSave'i yeniden tetiklemez, sonsuz döngü riski yok (bkz. kod
  yorumu `rule-engine-hook.ts`).

### FAZ 5.3 — `workflow_rules` Tablosu
- `db/schema.ts`: `workflowRules` (merkezi DB) — `entity_id, module_key, name, is_active,
  trigger (enum: on_create|on_update|on_stage_change|scheduled), conditions jsonb, actions jsonb`.
  **Bilinçli kapsam:** enum 4 trigger'ı kabul eder ama şu an yalnızca `on_create`/`on_update`
  hook'lara bağlı — `on_stage_change` Faz 6'nın transition takibini, `scheduled` bir cron
  producer'ı gerektiriyor, ikisi de yok.
- `db/migrations/0025_workflow_rules.sql` — **FAZ 4.1'de belgelenen konvansiyona uyuldu:**
  `docker exec -i ki_postgres psql ... < 0025_workflow_rules.sql` ile uygulandı, `npx
  drizzle-kit migrate`'e GÜVENİLMEDİ (journal 0011'de donmuş durumda).

### FAZ 5.4 — Deterministik Rule Evaluator
- `src/engine/rules/evaluator.ts`: `matchesConditions()` — saf fonksiyon, AND/OR ağacı +
  `{field,op,value}` yaprakları (`=,!=,>,>=,<,<=,contains,in`), kod-exec YOK. `evaluateRules(ctx)`
  eşleşen kuralların `actions[]`'ını `enqueueAction()` ile kuyruğa yazar — **AI burada karar
  vermez**, salt deterministik filtre.

### FAZ 5.5 — Action Queue & Handlers
- `src/workers/handlers/updateField.ts`: **güvenlik kritik** — `action.config.field` (admin
  tarafından `workflow_rules.actions` JSONB'sine yazılan, güvenilmeyen string) SQL'e DOĞRUDAN
  interpolate edilmez; FAZ 4.3'ün `getModuleSchema()` registry'sinden çözülen GERÇEK kolon
  adı kullanılır, bilinmeyen alan reddedilir. `||` merge ile custom field de yazabilir.
- `src/workers/handlers/webhook.ts`: `fetch` + 10sn timeout. **Bulunan ve düzeltilen bug:**
  GET/HEAD metodunda `body` göndermeye çalışınca "Request with GET/HEAD method cannot have
  body" hatası alındı (canlı testte yakalandı) — `hasBody` kontrolüyle düzeltildi.
- `src/workers/handlers/email.ts`: inline nodemailer (auth.ts'teki desenin aynısı — bu
  repo'da paylaşılan email helper'ı yok). **Not:** gerçek e-posta gönderimi canlı test
  edilmedi (yan etkisiz tutmak için) — kod review + tip kontrolüyle doğrulandı.
- `src/workers/handlers/runFunction.ts`, `requireApproval.ts`: Faz 7/6 köprü stub'ları (log +
  no-op, sonsuz retry'ı önlemek için throw etmiyor).
- **Doğrulama (gerçek HTTP + gerçek kuyruk + gerçek worker, tam pipeline):**
  1. `update_field`: `workflow_rules` satırı eklendi (`crm_deals`, `on_create`,
     `dealValue > 1000` → `leadSource = 'auto-flagged-highvalue'`). `POST /crm-native/deals`
     `dealValue:5000` ile çağrıldı → worker logunda `update_field tamamlandı` → DB'de
     `lead_source` gerçekten güncellenmiş bulundu.
  2. `webhook`: `crm_deals`/`on_create`/`title='Webhook Test Deal'` → `webhook` action,
     `http://ki_api:3001/health` (worker'ın aynı docker network'ünden gerçek hedef). İlk
     denemede yukarıdaki GET+body bug'ı yakalandı, düzeltildi, ikinci denemede
     `webhook tamamlandı` ile doğrulandı.
  3. Tüm 4 CRM modülünde (contacts/companies/deals/activities) create+update regresyon testi
     tekrarlandı — hook zinciri eklendikten sonra da hatasız.
  Test kuralları ve kayıtlar temizlendi.

**Faz 5 DoD karşılandı:** "Deal değeri > X ise webhook at + alan güncelle" kuralı UI'dan
