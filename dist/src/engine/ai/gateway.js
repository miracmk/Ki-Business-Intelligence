...
import { platformConfigs, kibiModelConfigs } from '../../../db/schema.js';
import { eq, and } from 'drizzle-orm';
import { decrypt } from '../../lib/crypto.js';
import { PROVIDERS, getConfigKey, parseModelString, KIBI_FREE_MODEL } from './providers.js';
...
function isModelUnavailable(errMsg, status) {
    if (status === 503 || status === 404 || status === 529)
        return true;
...
export class AiGateway {
    ...
    async complete(messages, opts = {}) {
        ...
        const err = new Error(`AI error (${model}): HTTP ${res.status} — ${errText}`);
        err.status = res.status;
        throw err;
    }
    // ── Fallback chain completion ─────────────────────────────────────────────
    async completeWithFallback(messages, modelList, opts = {}) {
        if (!modelList.length) {
        ...
        for (const model of models) {
            try {
                ...
                if (!unavail)
                    throw e; // auth / quota issues → bubble up
    ...
    static async fetchFreeModels(apiKey) {
...
async function cachedPlatformKey(cacheKey, dbKey) {
    const hit = keyCache.get(cacheKey);
    if (hit && Date.now() - hit.ts < KEY_TTL)
        return hit.key;
    try {
...
export function invalidateProviderKeyCache(dbKey) {
    if (dbKey) {
...
async function getKibiKey(providerId) {
...
async function getEntityFreeKey(providerId) {
    return cachedPlatformKey(`entity_free:${providerId}`, getConfigKey(providerId, 'entity_free'));
}
async function getEntityOwnKey(providerId, tenantId) {
    try {
        ...
        if (!config)
            return null;
        const providerKeys = config.settings?.providerKeys;
...
async function resolveKibiFreeModel() {
    try {
        const roles = await db.select().from(kibiModelConfigs)
            .where(and(eq(kibiModelConfigs.scope, 'entity_free'), eq(kibiModelConfigs.modelRole, 'conversation')));
        const row = roles[0];
        if (!row)
            return null;
...
function buildProviderHeaders(providerDef, apiKey) {
    const h = { 'Content-Type': 'application/json', ...providerDef.extraHeaders };
 ...
 */
export async function aiComplete(modelStr, messages, tenantId, opts = {}) {
    ...
    if (modelStr === KIBI_FREE_MODEL) {
        ...
        if (!resolved)
            throw new Error('KIBI Free Tier: entity_free scope için provider yapılandırılmamış');
        ...
        if (!parsed)
            throw new Error(`Geçersiz model string: ${modelStr} (beklenen: provider::modelId)`);
        ...
        if (isUUID(tenantId)) {
            ...
            if (!resolvedKey)
                resolvedKey = await getEntityFreeKey(resolvedProvider);
        }
        else {
    ...
    if (!providerDef)
        throw new Error(`Bilinmeyen provider: ${resolvedProvider}`);
 ...
 */
export async function aiEmbed(texts, modelStr, tenantId) {
    const inputs = Array.isArray(texts) ? texts : [texts];
    const parsed = parseModelString(modelStr);
    if (!parsed)
        throw new Error(`aiEmbed: geçersiz model string: ${modelStr}`);
    ...
    if (isUUID(tenantId)) {
        ...
        if (!apiKey)
            apiKey = await getEntityFreeKey(providerId);
    }
    if (!apiKey)
        apiKey = await getKibiKey(providerId);
    if (!apiKey)
        throw new Error(`aiEmbed: ${providerId} için API key bulunamadı`);
    const providerDef = PROVIDERS.find(p => p.id === providerId);
    if (!providerDef)
        throw new Error(`aiEmbed: bilinmeyen provider: ${providerId}`);
    if (!providerDef.supportsEmbeddings)
        throw new Error(`${providerDef.name} embedding desteklemiyor`);
    ...
    if (providerDef.needsAccountId) {
        ...
        if (!accountId || !realKey)
            throw new Error('Cloudflare: key formatı "accountId|apiKey" olmalı');
    ...
    if (providerId === 'cohere') {
        const vecs = data?.embeddings?.float;
        if (!vecs?.length)
            throw new Error('aiEmbed: Cohere boş embedding döndürdü');
        return vecs;
    }
    // OpenAI-compat: { data: [{ embedding: number[] }] }
    const items = data?.data;
    if (!items?.length)
        throw new Error('aiEmbed: boş embedding yanıtı');
