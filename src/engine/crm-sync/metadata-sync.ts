...
export interface MetadataSyncResult {
  ...
  relatedUpserted: number
  errors:          string[]
}

export async function runMetadataSync(connectionId: string): Promise<MetadataSyncResult> {
  const result: MetadataSyncResult = { modulesUpserted: 0, fieldsUpserted: 0, relatedUpserted: 0, errors: [] }
