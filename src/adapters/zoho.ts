...
import { CrmAdapter, type CrmCredentials, type CrmModule, type CrmRecord, type SearchParams, type BulkJob, type NotifSubscription } from './base.js'

export interface ZohoCreds extends CrmCredentials {
  type:          'zoho'
  clientId:      string
  clientSecret:  string
  refreshToken:  string
...
export class ZohoAdapter extends CrmAdapter {
  ...
  private get c(): ZohoCreds { return this.creds as ZohoCreds }

  private get apiBase()    { return `https://www.zohoapis.${this.c.region}/crm` }
  private get accountsBase() { return `https://accounts.zoho.${this.c.region}` }
  ...
  private async fetch<T>(path: string, version = 'v8', opts?: RequestInit): Promise<T> {
  ...
  async getModules(): Promise<CrmModule[]> {
    const data = await this.fetch<{ modules: Array<Record<string, unknown>> }>('/settings/modules')
    return (data.modules ?? [])
  ...
  async getModuleFields(module: string) {
    try {
      const data = await this.fetch<{ fields?: Array<Record<string, unknown>> }>(
      ...
      return (data.fields ?? [])
        ...
        .map((f) => ({
          ...
          maxLength:     f['length'] as number | undefined,
          pickListValues: f['pick_list_values'] as unknown[] | undefined,
          lookup:        f['lookup'],
          raw:           f,
        }))
    } catch { return [] }
  ...
  async getRelatedLists(module: string) {
    try {
      const data = await this.fetch<{ related_lists?: Array<Record<string, unknown>> }>(
        `/settings/related_lists?module=${module}`
      )
  ...
  async search({ module, criteria, fields, page = 1, perPage = 200 }: SearchParams): Promise<CrmRecord[]> {
    const params = new URLSearchParams({ page: String(page), per_page: String(perPage) })
    if (criteria) params.set('criteria', criteria)
    if (fields?.length) params.set('fields', fields.join(','))

    try {
      const data = await this.fetch<{ data?: Array<Record<string, unknown>> }>(
  ...
  async getRecord(module: string, id: string): Promise<CrmRecord | null> {
    try {
      const data = await this.fetch<{ data?: Array<Record<string, unknown>> }>(`/${module}/${id}`)
  ...
  async createRecord(module: string, data: Record<string, unknown>): Promise<CrmRecord> {
    const res = await this.fetch<{ data: Array<{ details: Record<string, unknown> }> }>(
      `/${module}`, 'v8', { method: 'POST', body: JSON.stringify({ data: [data] }) }
    )
  ...
  async updateRecord(module: string, id: string, data: Record<string, unknown>): Promise<CrmRecord> {
    const res = await this.fetch<{ data: Array<{ details: Record<string, unknown> }> }>(
      `/${module}/${id}`, 'v8', { method: 'PUT', body: JSON.stringify({ data: [data] }) }
    )
    return this.mapRecord(module, res.data[0]?.details ?? {})
  }
  ...
  async startBulkRead(module: string, callbackUrl: string): Promise<BulkJob> {
    const res = await this.fetch<{ data: Array<{ status: string; details: { id: string; state: string }; message?: string }> }>(
      '/bulk/read', 'bulk/v8',
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  async *downloadBulkResult(jobId: string): AsyncIterable<Record<string, unknown>> {
    // Step 1: download ZIP
    const token = await this.getToken()
  ...
  async subscribeNotifications(modules: string[], callbackUrl: string): Promise<NotifSubscription> {
    const channelId = Date.now()
    const expiry    = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString()

    const events = modules.flatMap((m) => [`${m}.create`, `${m}.edit`, `${m}.delete`])

    const res = await this.fetch<{ watch: Array<{ channel_id: number; expiry: string }> }>(
      '/actions/watch', 'v8',
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  async renewNotifications(channelId: string, callbackUrl: string): Promise<NotifSubscription> {
  ...
  // ── Internal helpers ──────────────────────────────────────────────────────
  private mapRecord(module: string, data: Record<string, unknown>): CrmRecord {
    ...
    return {
      ...
      modifiedTime: data['Modified_Time'] as string | undefined,
    }
  }
}

// ── ZIP extraction (Node 18+ DecompressionStream) ─────────────────────────────
async function unzipFirst(zipBytes: Uint8Array): Promise<string> {
