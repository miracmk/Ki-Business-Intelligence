...
SADECE JSON döndür: {"type":"crm-db","confidence":"high","reason":"..."}`
  ...
  // FAZ 10.1: the old "YFZ 19-21 / FAZ B — Connector AI Catalog + Analysis + Pipeline
  // Logging" block (catalog list/approve/bulk-approve, analyze/stream, test-query) lived
  // here — it only fed entity-db-engine.ts's natural-language-to-SQL querying against
  // AI-approved arbitrary tables. That whole path is retired: the wizard is now a one-time
  // Import into native CRM/ERP/Accounting tables (FAZ 10.5), and the AI reads/writes those
  // same native tables directly (FAZ 10.4) — there's no "catalog" to approve anymore.
}
