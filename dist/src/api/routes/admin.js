...
import { kibiEntities, kibiSupportTickets, kibiModelConfigs, kibiInternalUsers, users, platformConfigs, platformVectorDocs, aiPipelineLogs } from '../../../db/schema.js';
...
export const adminRoutes = async (app) => {
    ...
    app.get('/metrics', async () => {
        ...
        return {
            ...
            crmConnections: Number(crmCount.rows[0]?.count ?? 0),
    ...
    app.get('/token-usage', async (req) => {
        const { entityId, from, to, groupBy = 'entity' } = req.query;
        const conditions = [];
        if (entityId)
            conditions.push(`entity_id = '${entityId}'`);
        if (from)
            conditions.push(`used_at >= '${from}'`);
        if (to)
            conditions.push(`used_at <= '${to}'`);
    ...
    app.get('/support/tickets', async (req) => {
        ...
        const tickets = await db.query.kibiSupportTickets.findMany({
            where: (t, { and, eq }) => {
                const conds = [];
                if (q.status)
                    conds.push(eq(t.status, q.status));
                if (q.priority)
                    conds.push(eq(t.priority, q.priority));
                if (q.entityId)
                    conds.push(eq(t.entityId, q.entityId));
    ...
    app.put('/models/:role', async (req, reply) => {
        ...
        if (existing) {
            await db.update(kibiModelConfigs).set(patch)
                .where(and(eq(kibiModelConfigs.scope, 'platform'), eq(kibiModelConfigs.modelRole, role)));
        }
        else {
            await db.insert(kibiModelConfigs).values({ scope: 'platform', modelRole: role, ...patch });
        }
    ...
    app.get('/platform-settings', async (req) => {
        ...
        return {
            configs: rows.map(r => ({
                ...
                value: (role === 'admin' && r.value && !r.isSecret) ? (() => { try {
    ...
    app.put('/platform-settings/:key', async (req, reply) => {
        const role = req.user?.role;
        if (role !== 'admin')
            return reply.status(403).send({ error: 'Sadece admin düzenleyebilir' });
    ...
    app.delete('/platform-settings/:key', async (req, reply) => {
        if (req.user?.role !== 'admin')
            return reply.status(403).send({ error: 'Sadece admin silebilir' });
        const { key } = req.params;
    ...
    app.get('/platform-connections/:category', async (req, reply) => {
        if (req.user?.role !== 'admin')
            return reply.status(403).send({ error: 'Sadece admin görüntüleyebilir' });
    ...
    app.put('/entities/:entityId/plan', async (req, reply) => {
        ...
        await db.update(kibiEntities)
            .set({ planName: planName, updatedAt: new Date() })
    ...
    app.put('/ai-providers/:scope/:providerId', async (req, reply) => {
        if (req.user?.role !== 'admin')
            return reply.status(403).send({ error: 'Sadece admin düzenleyebilir' });
        const { scope, providerId } = req.params;
        const dbScope = urlScopeToDb(scope);
        if (!dbScope)
            return reply.status(400).send({ error: 'Geçersiz scope' });
    ...
    app.get('/ai-providers/:scope/models', async (req, reply) => {
        ...
        if (dbScope === 'kibi' && !configuredProviders.find(p => p.id === 'openrouter')) {
            ...
            if (legacyRow)
                configuredProviders.push({ id: 'openrouter', encryptedKey: legacyRow.value });
        }
        const results = [];
        for (const { id: providerId, encryptedKey } of configuredProviders) {
            ...
            if (cached) {
                try {
                    results.push({ provider: providerId, models: JSON.parse(cached) });
                    continue;
                }
            ...
            try {
                ...
                if (!modelsRes.ok) {
                    console.warn(`[AI-PROVIDERS] ${providerId} models fetch ${modelsRes.status}`);
                    continue;
                }
                ...
                if (providerId === 'openrouter') {
                    ...
                    models = (modelsData.data ?? []).map((m) => ({ id: m.id, name: m.display_name ?? m.id }));
                }
                else if (providerId === 'google') {
                    // Google returns { models: [{ name: 'models/gemini-...' }] }
                    models = (modelsData.models ?? modelsData.data ?? []).map((m) => {
                ...
                console.warn(`[AI-PROVIDERS] ${providerId} models fetch failed:`, e.message);
            }
        }
    ...
    app.put('/ai-providers/:scope/roles', async (req, reply) => {
        ...
        if (!roles || typeof roles !== 'object')
            return reply.status(400).send({ error: 'roles zorunlu' });
        const platformDbScope = dbScope === 'kibi' ? 'platform' : 'entity_free';
        for (const [role, assignment] of Object.entries(roles)) {
            if (!assignment.primary)
                continue;
            const existing = await db.query.kibiModelConfigs.findFirst({
                where: (t, { and, eq }) => and(eq(t.scope, platformDbScope), eq(t.modelRole, role)),
            });
            const patch = {
            ...
            if (existing) {
                await db.update(kibiModelConfigs).set(patch)
                    .where(and(eq(kibiModelConfigs.scope, platformDbScope), eq(kibiModelConfigs.modelRole, role)));
            }
            else {
                await db.insert(kibiModelConfigs).values({
                    scope: platformDbScope, modelRole: role, ...patch,
                });
            }
            invalidateModelCache(role);
        }
    ...
    app.post('/seed', async (req, reply) => {
        ...
        if (!user)
            return reply.status(500).send({ error: 'Unable to create admin' });
    ...
    app.get('/pipeline-logs', async (req, reply) => {
        const { role, entityId, modelRole, success, limit } = req.query;
        const filters = [];
        if (modelRole)
            filters.push(eq(aiPipelineLogs.modelRole, modelRole));
        if (entityId)
            filters.push(eq(aiPipelineLogs.entityId, entityId));
        if (success !== undefined)
            filters.push(eq(aiPipelineLogs.success, success === 'true'));
