...
export const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/register', async (req, reply) => {
    ...
    try {
      const { email, password, name, tenantName } = req.body as any
  ...
  app.post('/login', async (req, reply) => {
    ...
    if (!user) { await argon2.hash('dummy'); return reply.status(401).send({ error: 'Invalid credentials' }) }
    if (!await argon2.verify(user.passwordHash, password)) return reply.status(401).send({ error: 'Invalid credentials' })
  ...
  app.post('/otp/send', async (req, reply) => {
    ...
    const { userId, channel } = req.body as { userId: string; channel: 'whatsapp' | 'email' }
    if (!await redis.exists(redisKeys.pending2fa(userId))) return reply.status(403).send({ error: 'No active session' })
    if (await redis.get(redisKeys.otpCooldown(userId, channel))) return reply.status(429).send({ error: 'Wait before requesting again' })
    ...
    try {
      if (channel === 'whatsapp') await sendWhatsAppOtp(user.phone!, code)
      else await sendEmailOtp(user.email, code)
  ...
  app.post('/otp/verify', async (req, reply) => {
    const { userId, channel, code } = req.body as { userId: string; channel: string; code: string }
    if (!await redis.exists(redisKeys.pending2fa(userId))) return reply.status(403).send({ error: 'No active session' })

    const raw = await redis.get(redisKeys.otpRecord(userId, channel))
    if (!raw) return reply.status(400).send({ error: 'Code expired' })
    const rec = JSON.parse(raw)
    if (rec.attempts >= 5) { await redis.del(redisKeys.otpRecord(userId, channel)); return reply.status(400).send({ error: 'Too many attempts' }) }
    rec.attempts++
    await redis.setex(redisKeys.otpRecord(userId, channel), OTP_TTL, JSON.stringify(rec))

    if (!await argon2.verify(rec.hash, code)) return reply.status(401).send({ error: 'Invalid code' })
    await redis.del(redisKeys.otpRecord(userId, channel))
  ...
  app.post('/totp/verify', async (req, reply) => {
    ...
    if (!await redis.exists(redisKeys.pending2fa(userId))) return reply.status(403).send({ error: 'No active session' })
    const user = await db.query.users.findFirst({ where: (t, { eq }) => eq(t.id, userId) })
    if (!user?.totpSecret) return reply.status(400).send({ error: 'TOTP not configured' })
    authenticator.options = { digits: 6, step: 30, window: 1 }
    if (!authenticator.verify({ token, secret: user.totpSecret })) return reply.status(401).send({ error: 'Invalid code' })
    return completeLogin(app, userId, reply)
  })
  ...
  app.post('/totp/confirm', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    if (!secret) return reply.status(400).send({ error: 'Kurulum süresi doldu, tekrar başlatın' })

    authenticator.options = { digits: 6, step: 30, window: 1 }
    if (!authenticator.verify({ token: code, secret })) return reply.status(401).send({ error: 'Geçersiz kod' })
  ...
  app.post('/refresh', async (req, reply) => {
    ...
    if (!raw) return reply.status(401).send({ error: 'Invalid refresh token' })
  ...
  app.post('/logout', { onRequest: [app.authenticate] }, async (req, reply) => {
    const { refreshToken } = req.body as { refreshToken?: string }
    if (refreshToken) await redis.del(redisKeys.refreshToken(refreshToken))
    return reply.send({ ok: true })
  })
}

async function completeLogin(app: any, userId: string, reply: any) {
