...
export const fileRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/:id/download', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    if (!file) return reply.status(404).send({ error: 'Dosya bulunamadı' })
    if (!file.storagePath || !existsSync(file.storagePath)) return reply.status(404).send({ error: 'Dosya diskde bulunamadı' })
  ...
  app.delete('/:id', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    const file = await db.query.fileStorage.findFirst({ where: (t, { eq }) => eq(t.id, id) })
    if (!file) return reply.status(404).send({ error: 'Dosya bulunamadı' })
    if (file.storagePath && existsSync(file.storagePath)) unlinkSync(file.storagePath)
