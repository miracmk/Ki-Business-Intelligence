...
import { emailConfigs, kibiSupportTickets, kibiSupportMessages } from '../../db/schema.js';
import { decryptJson } from '../lib/crypto.js';
import { runAgent } from './ai/agent.js';
import nodemailer from 'nodemailer';
import { nanoid } from 'nanoid';
// Track per-entity poller timers
const pollerHandles = new Map();
async function pollEntityInbox(tenantId, creds) {
    if (!creds.imap?.host || !creds.imap?.user)
        return;
...
async function schedulePoller(tenantId, creds) {
...
export async function reloadTenantPoller(tenantId) {
...
//# sourceMappingURL=imap-poller.js.map