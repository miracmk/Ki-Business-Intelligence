...
import { kibiSupportTickets, kibiSupportMessages, kibiSupportAgents } from '../../../db/schema.js';
...
const ROLE_HIERARCHY = ['entity_external', 'entity_sub', 'entity_supervisor', 'entity_main', 'supervisor', 'admin'];
export const supportRoutes = async (app) => {
    app.get('/tickets', { onRequest: [app.authenticate] }, async (req) => {
        ...
        const tickets = await db.query.kibiSupportTickets.findMany({
            where: (t, { eq, and }) => {
                ...
                if (priority)
                    conditions.push(eq(t.priority, priority));
    ...
    app.post('/tickets', { onRequest: [app.authenticate] }, async (req, reply) => {
        const user = req.user;
        const { subject, serviceCategory, contactChannel, description } = req.body;
        ...
        const entity = await db.query.kibiEntities.findFirst({ where: (t, { eq }) => eq(t.entityId, user.tenantId) });
        if (!entity)
            return reply.status(404).send({ error: 'Entity bulunamadı. Lütfen admin ile iletişime geçin.' });
    ...
    app.get('/tickets/:id/kibi-response', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const ticket = await db.query.kibiSupportTickets.findFirst({ where: (t, { and, eq }) => and(eq(t.id, id), eq(t.entityId, entity.id)) });
        if (!ticket)
            return reply.status(404).send({ error: 'Ticket bulunamadı' });
    ...
    app.post('/tickets/:id/messages', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!content)
            return reply.status(400).send({ error: 'content gerekli' });
        ...
        if (senderType === 'agent' && ticket.externalContactId) {
    ...
    app.put('/tickets/:id/status', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!status)
            return reply.status(400).send({ error: 'status gerekli' });
        await db.update(kibiSupportTickets).set({ status: status }).where(and(eq(kibiSupportTickets.id, id), eq(kibiSupportTickets.entityId, entity.id)));
    ...
    app.put('/tickets/:id/priority', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!priority)
            return reply.status(400).send({ error: 'priority gerekli' });
        await db.update(kibiSupportTickets).set({ priority: priority }).where(and(eq(kibiSupportTickets.id, id), eq(kibiSupportTickets.entityId, entity.id)));
        return reply.send({ ok: true });
    });
    ...
    app.post('/tickets/:id/escalate', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!ticket)
            return reply.status(404).send({ error: 'Ticket bulunamadı' });
        const currentRole = (user.role ?? 'entity_sub');
        const currentIdx = ROLE_HIERARCHY.indexOf(currentRole);
        const nextRole = ROLE_HIERARCHY[currentIdx + 1];
        if (!nextRole)
            return reply.status(400).send({ error: 'Daha üst yönetim kademesi yok' });
    ...
    app.get('/admin/tickets', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const { status, entityId } = req.query;
        const tickets = await db.query.kibiSupportTickets.findMany({
            where: (t, { eq, and }) => {
                const conds = [];
                if (status)
                    conds.push(eq(t.status, status));
                if (entityId)
                    conds.push(eq(t.entityId, entityId));
    ...
    app.post('/agents', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!entity)
            return reply.status(404).send({ error: 'Entity bulunamadı' });
        const { channelPreference = 'email', waPhone, telegramChatId, notificationEmail, weight = 1 } = req.body;
    ...
    app.put('/agents/me', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!entity)
            return reply.status(404).send({ error: 'Entity bulunamadı' });
        const { channelPreference, waPhone, telegramChatId, notificationEmail, weight, isActive } = req.body;
