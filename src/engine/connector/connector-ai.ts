...
import type { ScannedTable, CatalogEntry } from './types.js'
...
Döndür (JSON):
{
  ...
  try {
    ...
    console.warn(`[CONNECTOR-AI] Parse failed for ${table.name}:`, (e as Error).message)
    return buildFallbackCatalog(table, rawTablePath)
  }
}

export async function runConnectorAnalysis(
  connectionId: string,
  entityId: string,
  entitySlug: string,
  sourceType: string,
    ...
    try {
      ...
      await db
        ...
        .values({
          ...
          columns: catalogEntry.columns as any,
          relationships: catalogEntry.relationships as any,
          queryTemplates: catalogEntry.queryTemplates as any,
          dataQuality: catalogEntry.dataQuality as any,
          rawTablePath,
...
export function buildRawTablePath(entitySlug: string, sourceType: string, tableName: string): string {
...
function buildFallbackCatalog(table: ScannedTable, rawTablePath: string): CatalogEntry {
  return {
    tableName: table.name,
