...
export interface RedisAnalysisResult {
  summary: string
  lastMood: string
...
export async function analyzeConversationHistory(params: { sessionKey: string; entityId?: string }): Promise<RedisAnalysisResult> {
  ...
  }).filter(Boolean).map((msg: any) => `${msg.role}: ${msg.content}`).join('\n')
