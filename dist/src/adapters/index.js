...
class NotImplementedAdapter extends CrmAdapter {
    ...
    async createRecord(m, d) { return { id: '', module: m, crm_id: '', crm_id_field: this.crmIdField, data: d }; }
    async updateRecord(m, id, d) { return { id, module: m, crm_id: id, crm_id_field: this.crmIdField, data: d }; }
    async deleteRecord() { }
    async startBulkRead() { return { jobId: '', module: '', status: 'failed' }; }
    async *downloadBulkResult() { }
...
export function createAdapter(creds) {
    switch (creds.type) {
        // ── CRM types ──
...
export function createAccountingAdapter(creds) {
    switch (creds.type) {
