...
export async function processNotification(
  ...
  const result: NotifResult = { processed: 0, deleted: 0, errors: [] }

  if (!payload.module || !payload.ids?.length) return result
  ...
  for (const id of payload.ids) {
    try {
      ...
      if (payload.operation === 'delete') {
        await db.delete(crmRecords).where(
          and(
            eq(crmRecords.connectionId, connectionId),
            eq(crmRecords.moduleApiName, payload.module),
            eq(crmRecords.crmId, id),
          )
        )
        result.deleted++
        continue
...
export async function setupNotifications(
  ...
  await db.update(db._.fullSchema.crmConnections as any)
    .set({
      notifChannelId:     sub.channelId,
      notifChannelExpiry: new Date(sub.expiresAt),
      updatedAt:          new Date(),
    })
    .where(eq(db._.fullSchema.crmConnections.id as any, connectionId))
...
async function loadConnection(connectionId: string) {
  ...
  return { ...conn, creds: decryptJson<CrmCredentials>(conn.credentials) }
}
