...
import { accountingConnections, accContacts, accInvoices, accPayments, accExpenses, paymentIntegrations, bankIntegrations } from '../../../db/schema.js';
...
export const accountingRoutes = async (app) => {
    ...
    app.post('/oauth/start', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const { provider, name, clientId, clientSecret, region = 'com', organizationId = '' } = req.body;
    ...
    app.post('/connections', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!check.ok)
            return reply.status(422).send({ error: `Bağlantı başarısız: ${check.error}` });
    ...
    app.get('/contacts', { onRequest: [app.authenticate] }, async (req) => {
        ...
        const contacts = await db.query.accContacts.findMany({
            where: (t, { and, eq, or }) => {
                const conditions = [eq(t.tenantId, user.tenantId)];
                if (type)
                    conditions.push(eq(t.contactType, type));
                if (search) {
    ...
    app.put('/contacts/:id', { onRequest: [app.authenticate] }, async (req) => {
        ...
        await db.update(accContacts).set({ ...body.data, updatedAt: new Date() }).where(eq(accContacts.id, id));
        return { ok: true };
    });
    ...
    app.get('/invoices', { onRequest: [app.authenticate] }, async (req) => {
        ...
        const invoices = await db.query.accInvoices.findMany({
            where: (t, { and, eq }) => {
                const conditions = [eq(t.tenantId, user.tenantId)];
                if (type)
                    conditions.push(eq(t.invoiceType, type));
                if (status)
                    conditions.push(eq(t.status, status));
                if (from)
                    conditions.push(sql `${t.issueDate} >= ${from}`);
                if (to)
                    conditions.push(sql `${t.issueDate} <= ${to}`);
                return and(...conditions);
            },
    ...
    app.put('/invoices/:id', { onRequest: [app.authenticate] }, async (req) => {
        ...
        if (!body.success)
            return { error: body.error.flatten() };
        await db.update(accInvoices).set({ ...body.data, updatedAt: new Date() }).where(eq(accInvoices.id, id));
        return { ok: true };
    });
    ...
    app.post('/invoices/upload', { onRequest: [app.authenticate] }, async (req, reply) => {
        if (!req.isMultipart || !req.isMultipart())
            return reply.status(400).send({ error: 'PDF dosyası gerekli' });
        const data = await req.file();
        if (!data)
            return reply.status(400).send({ error: 'Dosya alınamadı' });
    ...
    app.get('/invoices/:id/pdf', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!invoice?.filePath)
            return reply.status(404).send({ error: 'PDF bulunamadı' });
    ...
    app.get('/expenses', { onRequest: [app.authenticate] }, async (req) => {
        ...
        const expenses = await db.query.accExpenses.findMany({
            where: (t, { and, eq }) => {
                ...
                if (from)
                    conditions.push(sql `${t.expenseDate} >= ${from}`);
                if (to)
                    conditions.push(sql `${t.expenseDate} <= ${to}`);
                return and(...conditions);
            },
    ...
    app.put('/expenses/:id', { onRequest: [app.authenticate] }, async (req) => {
        ...
        if (!body.success)
            return { error: body.error.flatten() };
        await db.update(accExpenses).set(body.data).where(eq(accExpenses.id, id));
        return { ok: true };
    });
    ...
    app.get('/reports/income-statement', { onRequest: [app.authenticate] }, async (req) => {
        ...
        if (from)
            where.push(sql `issue_date >= ${from}`);
        if (to)
            where.push(sql `issue_date <= ${to}`);
    ...
    app.get('/reports/balance-sheet', { onRequest: [app.authenticate] }, async (req) => {
        ...
        const assets = await db.execute(sql.raw(`SELECT COALESCE(SUM(balance),0) AS assets FROM acc_contacts WHERE tenant_id = '${req.user.tenantId}'`));
    ...
    app.get('/reports/cash-flow', { onRequest: [app.authenticate] }, async (req) => {
        ...
        const income = await db.execute(sql.raw(`SELECT COALESCE(SUM(amount),0) AS operating FROM acc_payments WHERE tenant_id = '${req.user.tenantId}' AND payment_type = 'received' ${from ? `AND payment_date >= '${from}'` : ''} ${to ? `AND payment_date <= '${to}'` : ''}`));
        const expenses = await db.execute(sql.raw(`SELECT COALESCE(SUM(amount),0) AS operating FROM acc_payments WHERE tenant_id = '${req.user.tenantId}' AND payment_type = 'sent' ${from ? `AND payment_date >= '${from}'` : ''} ${to ? `AND payment_date <= '${to}'` : ''}`));
    ...
    app.get('/reports/profitability', { onRequest: [app.authenticate] }, async (req) => {
        const { from, to } = req.query;
        const data = await db.execute(sql.raw(`SELECT COALESCE(SUM(total),0) AS revenue FROM acc_invoices WHERE tenant_id = '${req.user.tenantId}' ${from ? `AND issue_date >= '${from}'` : ''} ${to ? `AND issue_date <= '${to}'` : ''}`));
        const expense = await db.execute(sql.raw(`SELECT COALESCE(SUM(amount),0) AS expenses FROM acc_expenses WHERE tenant_id = '${req.user.tenantId}' ${from ? `AND expense_date >= '${from}'` : ''} ${to ? `AND expense_date <= '${to}'` : ''}`));
        const revenue = Number(data.rows[0]?.revenue ?? 0);
