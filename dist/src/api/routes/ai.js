...
import { kibiTokenUsage, aiSessions } from '../../../db/schema.js';
import { eq, sql } from 'drizzle-orm';
import { getEntityDataSummary } from '../../lib/entity-provisioner.js';
import { env } from '../../../config/env.js';
async function resolveEntityId(userId, tenantId) {
...
const PLAN_MSG_LIMITS = {
    free: 100, starter: 500, growth: 2000, enterprise: 999999,
};
async function getEntityWithPlan(tenantId) {
    return db.query.kibiEntities.findFirst({
        where: (t, { eq }) => eq(t.entityId, tenantId),
        columns: { id: true, planName: true },
    }).catch(() => null);
}
async function trackTokenUsage(entityId, userId, modelName, promptTokens, completionTokens) {
...
export const aiRoutes = async (app) => {
    ...
    app.post('/chat', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const isUUIDCheck = (s) => !!s && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(s);
