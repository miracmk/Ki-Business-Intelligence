...
import { crmBulkJobs, crmRecords, crmSyncState, crmSyncLog } from '../../../db/schema.js';
...
export async function startBulkSync(connectionId, options) {
...
export async function processBulkCallback(connectionId, payload) {
    ...
    try {
        ...
        runEntityEtl(connectionId).catch(err => console.error(`[ETL] entity-etl failed for ${connectionId}:`, err));
    }
    catch (err) {
        await db.update(crmBulkJobs)
...
//# sourceMappingURL=bulk-sync.js.map