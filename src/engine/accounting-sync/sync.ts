...
import type { AccountingCredentials } from '../../adapters/accounting-base.js'

export async function syncAccounting(connectionId: string) {
  ...
  // Update sync state
  for (const type of ['invoice', 'payment', 'customer'] as const) {
...
async function loadConnection(connectionId: string) {
  ...
  const creds = decryptJson<AccountingCredentials>(conn.credentials)
  return { ...conn, creds }
}
