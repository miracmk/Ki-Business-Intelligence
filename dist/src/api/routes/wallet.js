...
import { kibiWallets, kibiWalletTransactions } from '../../../db/schema.js';
...
async function syncFromKiWallet(email, walletId) {
    try {
...
async function debitFromKiWallet(email, walletId, amountKiCoin, description) {
    try {
        const kiWalletUrl = env.KI_WALLET_URL ?? 'http://ki-wallet:3001';
...
export const walletRoutes = async (app) => {
    ...
    app.get('/', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (wallet) {
            ...
            if (live) {
                ...
                wallet = { ...wallet, balanceKiCoin: String(live.balanceKiCoin), balanceUsd: String(live.balanceUsd) };
            }
        }
        // Upcoming charges (last 7 days token usage estimate)
        const upcomingCharges = await db.query.kibiWalletTransactions.findMany({
            where: wallet ? (t, { and, eq, gte }) => and(eq(t.walletId, wallet.id), eq(t.type, 'charge')) : undefined,
    ...
    app.post('/register', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (existing)
            return reply.status(409).send({ error: 'Bu entity için zaten bir cüzdan kayıtlı' });
    ...
    app.get('/transactions', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!wallet)
            return reply.send({ transactions: [], wallet: null });
    ...
    app.post('/charge', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!wallet)
            return reply.status(404).send({ error: 'Bu entity için kayıtlı cüzdan yok' });
